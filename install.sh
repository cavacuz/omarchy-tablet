#!/bin/bash
# install.sh — tablet-companion share package installer (omarchy-tablet plugin).
...
-- GENERATED by tablet-companion install.sh — device names for tablet.lua.
...
# tablet-companion device overrides (sourced after auto-detection; wins).
...
-- tablet-companion package (additive standalone block — merged with any above).
hl.config({
  input = {
    touchpad = {
      natural_scroll = true,
...
echo "install.sh: ALL GREEN — re-run anytime; verify with 'tablet-verify.sh'"
...
USAGE_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/tablet-companion"
